		...
		for (let i=0; i<t.length; i++){
			...
			if (t[i] == '#'){
				...
				if (t[a] == ' '){
					...
					insert = `</p><a name="${t.slice(a+1, b)}" class="target" href="#${t.slice(a+1, b)}" style="text-decoration:none;"><h${count}>${encode(t.slice(a+1, b), true)}</h${count}></a><p>`;
			...
			if (t.slice(i, i+3) == '```'){
				...
				insert = Markdown.Code.encode(t.slice(a, b));
				// insert = "TBD";
